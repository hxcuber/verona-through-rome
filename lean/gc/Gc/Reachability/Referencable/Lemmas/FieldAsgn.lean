...
theorem fieldAsgn_corollary_stack_shape_eq (h : fieldAsgn x yf cfg = some cfg') (n : ℕ) :
...
theorem fieldAsgn_corollary_stack_length_eq (h : fieldAsgn x yf cfg = some cfg') :
...
theorem fieldAsgn_corollary_frame_transport_down (h : fieldAsgn x yf cfg = some cfg')
...
theorem fieldAsgn_corollary_frame_transport_up (h : fieldAsgn x yf cfg = some cfg')
    (fr : FrameWithIndex) (hfr : fr ∈ cfg.stackWithIndex) :
    ∃ fr0 : FrameWithIndex, fr0 ∈ cfg'.stackWithIndex ∧ fr0.index = fr.index ∧
      fr0.regionId = fr.regionId ∧ fr0.bridgeVar = fr.bridgeVar ∧ fr0.varMap = fr.varMap := by
  obtain ⟨fr0, hmem, hidx, hproj⟩ :=
...
-- one.
theorem fieldAsgn_corollary_heap_bridge_eq (h : fieldAsgn x yf cfg = some cfg') (rid0 : RegionId) :
...
theorem fieldAsgn_corollary_frameRoot_iff (h : fieldAsgn x yf cfg = some cfg')
...
theorem fieldAsgn_corollary_stack_objAt_mutated {cfg cfg' : RuntimeConfig} {frame0 : FrameWithIndex}
...
theorem fieldAsgn_corollary_stack_objAt_eq_of_ne {cfg : RuntimeConfig} {frame : FrameWithIndex}
...
theorem fieldAsgn_corollary_region_objAt_eq_of_ne {cfg : RuntimeConfig} (vcfg : ValidConfig cfg)
...
theorem fieldAsgn_corollary_region_objAt_mutated {cfg cfg' : RuntimeConfig} (vcfg : ValidConfig cfg)
...
-- Packaged here as its own reusable theorem since CR5 (`Gc/Reachability/Referencable/CR5/*.lean`) needs
