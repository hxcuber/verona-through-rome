...
-- report.pdf CR6 (`StackReachable_iff_RegionReachable_of_closed`) lives in `CR6.lean`, not here: it
-- needs `Lemmas/Common.lean`-level facts, which itself imports this file, so it can't live here too.
