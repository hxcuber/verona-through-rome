...
-- Generic in the resolved reference's shape (a field can hold an `RId` too, e.g. swap's
-- SWAP-STACK/SWAP-REGION-REGION branches write one into an object's field), unlike a plain
-- `FrameReachable` root, which is always specifically about some fixed target reference.
...
-- A chain starting at a bare RId (into a Closed region) can never reach an OId confirmed to live in a different, Open region -- the RId source forces a first hop into its own (Closed) region's bridge object, reducing to `reflTransGen_region_open_ne_absurd` one hop later.
...
-- Heap `bridgeObjectId` at any key transports across a region-objMap-only mutation (`hbv` witnesses the touched region's own `bridgeObjectId` is literally unaffected) -- used for FrameRoot's bridge disjunct, which never reads `objMap`.
...
-- A varMap-only change to the last frame never affects `objAt?` either -- mirrors `swap_corollary_stack_varmap_loc_eq` at the `objAt?` level.
...
-- The mutated container's own new content on the REGION-touching branches: computed directly from the region-level insert at the already-present key `yoid`. Generalized over `newBridge : ObjectId` so the same proof covers both an untouched `bridgeObjectId` (non-bridge callers pass `region.bridgeObjectId`) and SWAP-REGION-BRIDGE's simultaneous bridge change.
...
-- Composes the region objMap change with the (irrelevant, since it never touches objMap) simultaneous stack varMap change -- needed because SWAP-REGION-OBJECT/SWAP-REGION-REGION change both simultaneously.
...
-- swap's stack-mutating branches (SWAP-STACK/SWAP-REGION-OBJECT/SWAP-REGION-REGION) all replace the last frame with some `newFrame` built via `{ frame with varMap := ..., objMap := ... }`, so `newFrame.regionId`/`newFrame.bridgeVar` always literally equal `frame.regionId`/`frame.bridgeVar` -- this packages the common index-case-split argument once.
...
-- Any frame membership in cfg'.stackWithIndex transports down to cfg with the SAME index, regionId and bridgeVar (though possibly a different varMap/objMap).
...
-- `FrameReachable` is completely unaffected at any position strictly before the mutated (last) frame, across all four swap branches -- a chain rooted at a suspended frame can never reach the mutated container `yoid` at all (owner-index bound), so no escape/root-escape machinery is needed here, just a per-branch FrameRoot-iff plus a straightforward tail-induction transport (handling both `OId`- and `RId`-sourced hops, since unlike `RefStep`, `ReachableStep` can be sourced from an `RId`).
theorem swap_frame_reachable_iff_of_lt {cfg cfg' : RuntimeConfig} (vcfg : ValidConfig cfg) (vcfg' : ValidConfig cfg')
  ...
  · -- SWAP-STACK
    ...
    have hoidm_ne : ∀ oidx, FrameReachable cfg frame.index (Reference.OId oidx) → oidx ≠ yoid := by
      ...
      rw [heq] at hreachx
      have := fieldAsgn_stack_container_confined vcfg hyloc0 hreachx
  ...
  · -- SWAP-REGION-OBJECT
    ...
    have hoidm_ne : ∀ oidx, FrameReachable cfg frame.index (Reference.OId oidx) → oidx ≠ yoid := by
      intro oidx hreachx heq
      rw [heq] at hreachx
      have := fieldAsgn_region_container_confined vcfg hregion hstatus hframe0_mem hrid.symm hyloc0 hreachx
      exact absurd this (Nat.not_le.mpr hltd)
    have hoidm_ne' : ∀ oidx, FrameReachable cfg' frame.index (Reference.OId oidx) → oidx ≠ yoid := by
      intro oidx hreachx heq
      rw [heq] at hreachx
      ...
      have hownerbound := fieldAsgn_region_container_confined vcfg' hlookup' hstatus hfr0 hfr0rid hyloc0' hreachx
  ...
  · -- SWAP-REGION-BRIDGE
    ...
    have hoidm_ne : ∀ oidx, FrameReachable cfg frame.index (Reference.OId oidx) → oidx ≠ yoid := by
      intro oidx hreachx heq
      rw [heq] at hreachx
      have := fieldAsgn_region_container_confined vcfg hregion hopenReg hframe0_mem hyridEq.symm hyloc0 hreachx
      exact absurd this (Nat.not_le.mpr hltd)
    have hoidm_ne' : ∀ oidx, FrameReachable cfg' frame.index (Reference.OId oidx) → oidx ≠ yoid := by
      intro oidx hreachx heq
      rw [heq] at hreachx
      ...
      have hlookup' : cfg'.heap.lookup yrid = some newRegionB := by
        rw [hcfg']; dsimp only; exact AList.lookup_insert cfg.heap
      have hownerbound := fieldAsgn_region_container_confined vcfg' hlookup' hopenReg hframe0_mem' hyridEq.symm
        hyloc0' hreachx
      exact absurd hownerbound (Nat.not_le.mpr hltd)
    have hobjAtEq : ∀ oid', oid' ≠ yoid →
        (Reference.OId oid').objAt? cfg = (Reference.OId oid').objAt? cfg' := by
...
-- ROOT escape, shared by all four `stackReachable_invariant_swap` branches: whatever the mutation places at the var/bridge root (`yfRef`, the field access `yf`'s OLD value) was already resolvable pre-mutation via `resolveFA`, so a chain starting there already has a natural pre-mutation root `frameY`, from which `frameD` is reached either directly (if `frameY = frameD`) or by recursing into `hcr3` itself.
theorem swap_escape {cfg : RuntimeConfig} (vcfg : ValidConfig cfg)
  ...
  | OId oidw =>
    ...
    have hownerbound := fieldAsgn_region_container_confined vcfg hlkD hopenD hframeDMem rfl hlocDown hreachY
...
-- Shared by `main_claim`'s SWAP-STACK/SWAP-REGION-OBJECT/SWAP-REGION-BRIDGE branches (the `OId`-shaped mutated-container-content case): a mid-chain hop using `newVal` already has `frame0` itself as a valid root (`hxRoot`), so it reaches `frameD` either directly or by recursing into `hcr3`.
...
-- The genuinely new case relative to `Gc.Reachability.Referencable`: unlike `RefStep`, `ReachableStep` CAN continue from an `RId` source (into a Closed region's bridge object), so a mid-chain hop landing on an `RId`-shaped `newVal` (SWAP-STACK's general `xRef`, SWAP-REGION-REGION's `RId xrid`) isn't vacuous the way it is there -- but reaching `oid` (confirmed to live in `frameD`'s own Open region) from an `RId` source is still impossible, via `reflTransGen_rid_source_open_absurd`.
...
-- The mutated container's own `ReachableStep`, restricted to `c ≠ newVal` (STACK branch): the new refs are exactly the old refs with `newVal` possibly added.
...
-- Mirrors `swap_stack_yoid_step_of_ne` for the REGION-touching branches (SWAP-REGION-OBJECT/SWAP-REGION-REGION, whose mutated container lives in a region, with a simultaneous but irrelevant stack varMap change).
...
-- `ReachableStep` agrees between `cfg`/`cfg'` for any source *other than* the mutated container's own `OId` (STACK branch): unconditional in the target, handling both `OId`- and `RId`-sourced hops (the heap is untouched entirely, so `RId` is trivial here).
...
-- Mirrors `swap_stack_step_iff_of_ne` for the REGION-touching branches (SWAP-REGION-OBJECT/SWAP-REGION-REGION): the touched region is always Open (its owning frame is on-stack, L2), so an `RId`-sourced hop out of it is impossible in either config.
